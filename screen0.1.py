...
def parse ():
    ...
    for n in nodelist:
        if n.left==None and n.right==None:
            continue
        if n.left==None:
            dict_11[n.name]=[n.right.name]
        elif n.right==None:
            dict_11[n.name]=[n.left.name]
        else:
            dict_11[n.name]=[n.left.name,n.right.name]
    print (dict_11)
    ...
    write_dot(G,'syntax.dot')
    plt.title('draw_syntax TREE')
    pos =graphviz_layout(G, prog='dot')
    nx.draw(G, pos, with_labels=True, arrows=False,node_size=640)
    plt.savefig('nx_syntax.png')
    print("k")
    #img=PhotoImage(file='nx_test.png')
    #image_label=Label(root,image=img)
    img.config(file='nx_syntax.png')
    image_label.config(image=img)
    image_label.place(x=550,y=100)
    leaves=[]
    #for x in G.nodes():
